...
class dBug {
	...
	//constructor
	function dBug($var,$forceType="",$bCollapsed=false, $output=true) {
	...
	function makeTableHeader($type,$header,$colspan=2) {
		...
		//echo 
        $this->salida .= "<table cellspacing=2 cellpadding=3 class=\"dBug_".$type."\">
				<tr>
	...
	function checkType($var) {
		switch(gettype($var)) {
			...
			default:
				...
				//echo 
                $this->salida .= "<table cellspacing=0><tr>\n<td>".$var."</td>\n</tr>\n</table>\n";
				break;
		}
	}
	...
	function varIsArray($var) {
		...
		if(is_array($var)) {
			foreach($var as $key=>$value) {
				...
				//echo 
                $this->salida .= $this->closeTDRow();
			}
		}
		else //echo 
        $this->salida .= "<tr><td>".$this->error("array").$this->closeTDRow();
	...
	function varIsObject($var) {
		...
		if(is_object($var)) {
			...
			foreach($arrObjMethods as $key=>$value) {
				...
				//echo 
                $this->salida .= "[function]".$this->closeTDRow();
			}
		}
		else //echo 
        $this->salida .= "<tr><td>".$this->error("object").$this->closeTDRow();
		array_pop($this->arrHistory);
		//echo 
        $this->salida .= "</table>";
	}
	...
	function varIsResource($var) {
		...
		switch(get_resource_type($var)) {
			...
			default:
				//echo 
                $this->salida .= get_resource_type($var).$this->closeTDRow();
				break;
		}
		//echo 
        $this->salida .= $this->closeTDRow()."</table>\n";
	}
	...
	function varIsDBResource($var,$db="mysql") {
		...
		//echo 
        $this->salida .= "<tr><td class=\"dBug_resourceKey\">&nbsp;</td>";
		for($i=0;$i<$numfields;$i++) {
			...
			//echo 
            $this->salida .= "<td class=\"dBug_resourceKey\" title=\"".$field_header."\">".$field_name."</td>";
		}
		//echo 
        $this->salida .= "</tr>";
		for($i=0;$i<$numrows;$i++) {
	...
	function varIsGDResource($var) {
		...
		//echo 
        $this->salida .= imagesx($var).$this->closeTDRow();
		$this->makeTDHeader("resource","Height");
		//echo 
        $this->salida .= imagesy($var).$this->closeTDRow();
		$this->makeTDHeader("resource","Colors");
		//echo 
        $this->salida .= imagecolorstotal($var).$this->closeTDRow();
		//echo 
        $this->salida .= "</table>";
	}
	...
	function varIsXmlResource($var) {
		...
		//if xml is not a file, attempt to read it as a string
			...
			if(!is_string($var)) {
				//echo 
                $this->salida .= $this->error("xml").$this->closeTDRow()."</table>\n";
				return;
			}
			$data=$var;
	...
	function xmlEndElement($parser,$name) {
		for($i=0;$i<$this->xmlCount;$i++) {
			...
			//echo 
            $this->salida .= (!empty($this->xmlCData[$i])) ? $this->xmlCData[$i] : "&nbsp;";
			//echo 
            $this->salida .= $this->closeTDRow();
			$this->makeTDHeader("xml","xmlComment");
			//echo 
            $this->salida .= (!empty($this->xmlDData[$i])) ? $this->xmlDData[$i] : "&nbsp;";
			//echo 
            $this->salida .= $this->closeTDRow();
