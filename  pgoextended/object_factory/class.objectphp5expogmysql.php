...
class Object
{
	...
	// -------------------------------------------------------------
	function Object($objectName, $attributeList = '', $typeList ='', $getterList ='', $setterList ='', $encryptList ='', $pdoDriver = '', $language = 'php5')
	{
	...
	function BeginObject()
	{
        ...
        $this->string.="private \$_".strtolower($this->objectName)."Id = '';\n\n\t";
		$x = 0;
		foreach ($this->attributeList as $attribute)
		{
			if ($this->typeList[$x] == "BELONGSTO")
			{
				...
				$this->string .=" */\n\t";
				//Modificado 2008-08-22 - Damian Eiff
                //Todas las variables pasan a ser privadas para poder controlar el acceso a las mismas
                //$this->string.="public $".strtolower($attribute)."Id;\n\t";                
                $this->string.="private \$_".strtolower($attribute)."Id;\n\t";
				$this->string.="\n\t";
			}
			...
			else
			{
                ...
                $this->string.="private \$_".$attribute.";\n\t";
				$this->string.="\n\t";
			}
			$x++;
		}
        ...
        //Agregar los datos adicionales del generador
        $this->string .= "public \$pog_attribute_type_ex = array(\n\t\t";
		$this->string .= "\"".strtolower($this->objectName)."Id\" => array('db_attributes' => array(\"NUMERIC\", \"INT\"),\n'pog_attributes' => array(true, false, false) ),\n\t\t";
		$x = 0;
		foreach ($this->attributeList as $attribute)
		{
			$this->string .= "\"".$attribute."\" => array('db_attributes' => array(\"".$misc->InterpretType($this->typeList[$x])."\", \"".$misc->GetAttributeType($this->typeList[$x])."\"".(($misc->InterpretLength($this->typeList[$x]) != null) ?  ', "'.$misc->InterpretLength($this->typeList[$x]).'"' : '')."),\n'pog_attributes' => array( 'getter' => " .$this->getterList[$x] . ", 'setter' => " . $this->setterList[$x] . ", 'encrypt' => " . $this->encryptList[$x] . ")),\n\t\t";
			$x++;
		}
		$this->string .= ");\n\t";
	}
	...
	function CreateConstructor()
	{
		...
		foreach ($this->attributeList as $attribute)
		{
			if ($this->typeList[$i] != "BELONGSTO" && $this->typeList[$i] != "HASMANY" && $this->typeList[$i] != "JOIN" && $this->setterList[$i] )
			{
				if ($j == 0)
				{
