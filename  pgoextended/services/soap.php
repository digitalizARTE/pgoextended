...
$server -> wsdl -> addComplexType(
    ...
    array(),
    array(
        array('ref'=>'SOAP-ENC:arrayType','wsdl:arrayType'=>'xsd:boolean')
    ),
    'xsd:boolean'
);
...
$server -> register('GenerateObjectEx',
					array('objectName' => 'xsd:string',
						'attributeList' => 'tns:StringArray',
						'typeList' => 'tns:StringArray',
                        'getterList' => 'tns:BoolArray',
                        'setterList' => 'tns:BoolArray',
                        'encryptList' => 'tns:BoolArray',
                        'insertList' => 'tns:BoolArray',
                        'updateList' => 'tns:BoolArray',
                        'defaultListPHP' => 'tns:BoolArray',
                        'defaultListDB' => 'tns:BoolArray',
                        'booleanList' => 'tns:BoolArray',                        
						'language' => 'xsd:string',
						'wrapper' => 'xsd:string',
						'pdoDriver' => 'xsd:string'),
					array('return' => 'xsd:string'),
					'urn:pogwsdl',
					'urn:pogwsdl#GenerateObjectEx',
					'rpc',
					'encoded',
					'Generates the appropriate object from supplied attributeList, typeList etc.'
					);                    
...
$server -> register('GeneratePackageEx',
					array('objectName' => 'xsd:string',
						'attributeList' => 'tns:StringArray',
						'typeList' => 'tns:StringArray',
                        'getterList' => 'tns:BoolArray',
 ...
 * @param array $attributeList
 * @param array $typeList
 * @param array $getterList
 * @param array $setterList
 ...
 * @return base64 encoded string
 */  
function GenerateObjectEx($objectName, 
                          $attributeList, 
                          $typeList, 
                          $getterList, 
                          $setterList, 
                          ...
                          $pdoDriver)
{
    ...
    */
    ...
    /*
    for($iq=0;$iq<count($getterList);$iq++)
    {
        //$getterList[ $iq ] = eval( $getterList[ $iq ] );
        $lcexp = ' $getterList[ '.$iq.' ] = '.( ( $getterList[ $iq ] ) ? 'true' :'false' ).';';
        eval( $lcexp );
    }    
            
    for($iq=0;$iq<count($setterList);$iq++)
    {
        //$setterList[ $iq ] = eval( $setterList[ $iq ] );
        $lcexp = ' $setterList[ '.$iq.' ] = '.( ( $setterList[ $iq ] ) ? 'true' :'false' ).';';
        eval( $lcexp );
    }
    
    for($iq=0;$iq<count($encryptList);$iq++)
    {
        //$encryptList[ $iq ] = eval( $encryptList[ $iq ] );
        $lcexp = ' $encryptList[ '.$iq.' ] = '.( ( $encryptList[ $iq ] ) ? 'true' :'false' ).';';
        eval( $lcexp );
    }
       
    for($iq=0;$iq<count($insertList);$iq++)
    {
        //$insertList[ $iq ] = eval( $insertList[ $iq ] );
        $lcexp = ' $insertList[ '.$iq.' ] = '.( ( $insertList[ $iq ] ) ? 'true' :'false' ).';';
        eval( $lcexp );
    }
       
    for($iq=0;$iq<count($updateList);$iq++)
    {
        //$updateList[ $iq ] = eval( $updateList[ $iq ] );
        $lcexp = ' $updateList[ '.$iq.' ] = '.( ( $updateList[ $iq ] ) ? 'true' :'false' ).';';
        eval( $lcexp );
    }
    
    for($iq=0;$iq<count($booleanList);$iq++)
    {
        //$booleanList[ $iq ] = eval( $booleanList[ $iq ] );
        $lcexp = ' $booleanList[ '.$iq.' ] = '.( ( $booleanList[ $iq ] ) ? 'true' : 'false' ).';';
        eval( $lcexp );
    }
    */
	if (strtoupper($wrapper) == "PDO")
	...
	else
	{
		...
		{
			// require_once "../object_factory/class.objectphp5expogmysql.php";
            require_once "../object_factory/class.objectphp5.1pdomysql.php";            
		}
	}
    
	/*
    $object = new Object( $objectName, $attributeList, $typeList, 
                         $getterList, $setterList, $encryptList, 
                         $insertList, $updateList, $defaultListPHP, $defaultListDB, 
                         $pdoDriver, $language );
    */
...
function GeneratePackageEx($objectName, 
                          $attributeList, 
                          $typeList, 
                          $getterList, 
                          $setterList, 
                          ...
                          $db_encoding = 0)
{
	...
	// $package["objects"]["class.".strtolower($objectName).".php"] =  GenerateObject($objectName, $attributeList, $typeList, $language, $wrapper, $pdoDriver);
    $package["objects"]["class.".strtolower($objectName).".php"] =  GenerateObjectEx($objectName, 
                                                                                    $attributeList, 
                                                                                    $typeList, 
                                                                                    $getterList, 
                                                                                    $setterList, 
